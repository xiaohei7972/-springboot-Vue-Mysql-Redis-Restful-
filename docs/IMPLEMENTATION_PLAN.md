...
| 5. 教学业务 API | 学生、院系、班级、教师、课程、选课、成绩、考勤、通知和统计 | REST `/api` 接口 + Service 业务层 | 核心业务已完成 | Controller 仅负责接口适配，业务权限和成绩计算由 Service 统一处理；教师课程、学生已选课程和学生可选课程均按角色隔离 |
| 6. 前端页面 | 登录、首页、数据列表、表单、分页、权限菜单和个人中心 | Vue 3、TypeScript、Element Plus、Pinia、Axios | 核心交互已完成 | 三类角色菜单和操作已匹配后端权限；管理员可维护教学基础数据，教师可录入成绩/考勤，学生可选课/退选，表单校验、下拉引用数据和个人设置已接入 |
| 7. 联调与测试 | 后端构建、前端构建、接口测试、权限测试和缓存测试 | Maven、npm、Spring Boot Test | 自动化测试完成 | 12 个后端单元测试通过，前端类型检查和生产构建通过，OpenAPI 快照校验通过；REST 冒烟测试待启动 MySQL、Redis 和后端后执行 |
| 8. 课程交付 | README、API 说明、演示账号、论文素材、答辩流程 | Markdown、OpenAPI JSON、截图素材 | 材料初稿完成 | 已补充 API 文档、OpenAPI 快照、架构图、功能模块图、数据库关系图、论文目录和答辩流程；仍需结合实际运行环境补充截图和最终测试结果 |
